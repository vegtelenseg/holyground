...
export const AuthContext = React.createContext<AuthContextStateType>(AuthContextState)
...
export function AuthController({ children }: Props) {
  ...
  const handleLogin = React.useCallback(
    async (userInfo: { password: string; email: string }) => {
      try {
        const { data } = await axios.post(`${process.env.REACT_APP_API_HOST}/auth/local`, {
